...
'''
def find_max(r):
    if not r.sx and not r.sx: return r.valore
    if not r.sx: return find_max(r.dx)
    if not r.dx: return find_max(r.sx)
...
'''
def height_tree(r):
    if not r.sx and not r.dx: return 0
    if not r.sx: return 1 + height_tree(r.dx)
    if not r.dx: return 1 + height_tree(r.sx)
