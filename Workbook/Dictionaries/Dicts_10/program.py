...
def es29(tabella1, tabella2, col):
    ...
    - una stringa con il nome di una delle colonne delle due tabelle rispetto 
    alla quale le due tabelle sono 
      ordinate in modo crescente.
    modifica distruttivamente la tabella 1.
    Le righe della tabella 1 differiscono tutte nella colonna col  e lo stesso 
    vale per le righe della tabella 2.
    Bisogna inserire nella tabella 1 righe della tabella 2. 
    Una riga della tabella 2 deve essere  inserita nella tabella 1 solo se nella
    sua colonna  col contiene un valore
    non presente tra quelli che compaiono nella corrispondente colonna della tabella1. 
    Al termine la tabella 1 deve ancora risultare ordinata rispetto ai valori 
    della colonna col.  
    ...
    add = []
    for elem in tabella2:
        if elem[col] not in values:
            add.append(elem)
    result = len(add)
    tabella1.extend(add)
