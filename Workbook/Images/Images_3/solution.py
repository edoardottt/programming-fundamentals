...
def es4(fimm, fimm1, h1, w1):
    ...
    for _ in range(
        hOrignale * h1
    ):  # preparo l'mmagine inserendo al posto di ogni pixel, uno 0
        row = []
    ...
    for x in range(hOrignale):
        for y in range(wOriginale):
            ...
            imgNuova = disegnaRettangolo(
                imgNuova, x * h1, y * w1, h1, w1, colorePix
            )  # disegna il rettangolo in imgNuova
    ...
    maxColor = max(
        countColor.values()
    )  # prendo il numero di volte maggiore che un colore compare
    listColor = sorted(
        list(filter(lambda x: countColor[x] == maxColor, countColor))
    )  # elimino tutti i colori che non compaiono il numero di volte massimo e ordino la lista ottenuta
