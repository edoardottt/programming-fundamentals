...
def es39(n, jsonFile):
    ...
    direzioni = (
        ...
        [0, -1],
        [-1, 0],
    )  # elenco delle 4 possibili direzioni (incrementi di y e di x)
    dir = 0  # la direzione iniziale e' la prima (verso destra)
    for i in range(1, n * n + 1):  # scandisco i valori da 1 a n^2 compreso
        ...
        if not (
            0 <= nx < n and 0 <= ny < n and matrice[ny][nx] == 0
        ):  # se la casella NON esiste o NON contiene 0
            dir = (dir + 1) % 4  # cambio direzione passando alla prossima
            ny = y + direzioni[dir][0]  # e ricalcolo ny e nx
            nx = (
                x + direzioni[dir][1]
            )  # per costruzione sono certo che la casella esiste ed e' vuota
    ...
    for x in range(0, n, 2):  # sommo le colonne con x pari
        for y in range(n):
            somma += matrice[y][x]
    with open(
        jsonFile, mode="w", encoding="utf8"
    ) as f:  # e salvo la matrice nel file json
        json.dump(matrice, f)
    return somma
