...
def es22(filePng, fileJson):
    ...
    diz={}
    for elem in freq:
        if freq[elem] not in diz: diz[freq[elem]] = [elem]
        else: diz[freq[elem]].append(elem)
    with open(fileJson,'w+') as f:
        json.dump(imm,f)
    return sorted(diz[max(diz.keys())])[0]
...
def encode(color):
    ...
    ga = str(g)
    ba = str(b)
    while len(ra)!=3: ra = ''.join(['0',ra])
    while len(ga)!=3: ga = ''.join(['0',ga])
    while len(ba)!=3: ba = ''.join(['0',ba])
    return ra+ga+ba