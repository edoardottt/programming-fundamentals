def es24(ifile, l):
    ...
    with open(ifile) as f:
        text = f.read()
    ls = []
    tot = len([i for i in text if i.isalpha()])
    for elem in l:
        item = ((text.lower().count(elem.lower())) / tot) * 100
        ls.append((elem, "{:.2f}%".format(item)))
    return sorted(ls, key=lambda x: (-float(x[1][:-1]), x[0]))
