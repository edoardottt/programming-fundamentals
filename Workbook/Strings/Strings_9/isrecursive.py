...
def norecurse(func):
    ...
    def f(*args, **kwargs):
        if DEBUG:
            print("calling", func.__name__, func.called, *args)
        if func.called:
        ...
        func.called = False
        if DEBUG:
            print("returning", result, "from", func.__name__)
        return result
...
def undecorate_module(module):
    ...
    for f in dir(module):
        ...
        if isinstance(ff, types.FunctionType):
            if DEBUG > 1:
                print("undecorating", f)
            ff = undecorate_function(ff)
            setattr(module, f, ff)
        elif isinstance(ff, type):
            if DEBUG > 1:
                print("undecorating", f, "methods")
            for m in dir(ff):
                if DEBUG > 1:
                    print("   undecorating", m)
                mm = getattr(ff, m)
                if isinstance(mm, types.FunctionType):
