...
class Disegno:
    ...
    def __repr__(self):
        return 'Disegno largo {} pixel e alto {} pixel, di colore {}'.format(self.larghezza, self.altezza, self.sfondo)

    def disegna_rettangolo(self, rettangolo, colore):
        if type(rettangolo) != Rettangolo:
            raise TypeError('Disegno solo rettangoli')
        for y in range(rettangolo.vas.y,
                       rettangolo.vbs.y+1):
            for x in range(rettangolo.vas.x,
                           rettangolo.vad.x+1):
